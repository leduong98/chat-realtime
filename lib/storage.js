...
export function loadSelfName() {
  if (typeof window === "undefined") return "";
  return window.localStorage.getItem(STORAGE_KEYS.SELF_NAME) || "";
}

export function saveSelfName(name) {
  if (typeof window === "undefined") return;
  const v = String(name || "").trim();
  if (!v) {
    window.localStorage.removeItem(STORAGE_KEYS.SELF_NAME);
    return;
  }
  window.localStorage.setItem(STORAGE_KEYS.SELF_NAME, v);
}
...
/** Lấy tin nhắn cuối của một cuộc hội thoại (để hiển thị preview trong sidebar). */
export async function getLastMessage(chatId) {
  const list = await loadMessages(chatId);
