...
export function createPollClient({ userId, onMessage, onStatus, intervalMs }) {
  ...
  let timer = null;
  let busy = false;
  const baseInterval = typeof intervalMs === "number" ? intervalMs : 250;
  let backoff = 0;
  ...
  function schedule() {
    ...
    // Long polling: usually return immediately on message, otherwise ~25s.
    // After each response, re-issue quickly; on errors, backoff.
    const wait = backoff ? Math.min(8000, 400 * backoff) : baseInterval;
